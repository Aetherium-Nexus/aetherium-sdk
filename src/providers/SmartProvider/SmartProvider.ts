...
import { AetheriumEtherscanProvider } from "./AetheriumEtherscanProvider.js";
import { AetheriumJsonRpcProvider } from "./AetheriumJsonRpcProvider.js";
...
export function getSmartProviderErrorMessage(errorMsg: string): string {
  return `${errorMsg}: RPC request failed. Check RPC validity. To override RPC URLs, see: https://docs.aetherium-nexus.com/docs/deploy-aetherium-troubleshooting#override-rpc-urls`;
}
...
type AetheriumProvider = AetheriumEtherscanProvider | AetheriumJsonRpcProvider;

export class AetheriumSmartProvider
  ...
  implements IProviderMethods
{
  ...
  public readonly explorerProviders: AetheriumEtherscanProvider[];
  public readonly rpcProviders: AetheriumJsonRpcProvider[];
  ...
  constructor(
      ...
      this.explorerProviders = blockExplorers
        .map((explorerConfig) => {
          if (
            ...
            const newProvider = new AetheriumEtherscanProvider(
              explorerConfig,
              network
            );
        ...
        .filter((e): e is AetheriumEtherscanProvider => !!e);
    } else {
    ...
    if (rpcUrls?.length) {
      this.rpcProviders = rpcUrls.map((rpcConfig) => {
        const newProvider = new AetheriumJsonRpcProvider(rpcConfig, network);
  ...
  isExplorerProvider(p: AetheriumProvider): p is AetheriumEtherscanProvider {
  ...
  protected async performWithFallback(
    ...
    providers: Array<AetheriumEtherscanProvider | AetheriumJsonRpcProvider>,
    reqId: number
  ): Promise<any> {
    let pIndex = 0;
