...
import { ERC20__factory } from "@aetherium-nexus/core";
...
describe("SmartProvider", async () => {
  let signer: Wallet;
  let smartProvider: AetheriumSmartProvider;
  let contractAddress: string;

  before(async () => {
    smartProvider = AetheriumSmartProvider.fromRpcUrl(NETWORK, URL, {
      maxRetries: 3,
    });
  ...
  it("throws with invalid RPC", async () => {
    ...
    const smartProvider = AetheriumSmartProvider.fromRpcUrl(
      INVALID_NETWORK,
      INVALID_URL
    );
  ...
  it("throws with multiple invalid RPCs", async () => {
    ...
    const INVALID_NETWORK = 55555;
    const smartProvider = new AetheriumSmartProvider(
      INVALID_NETWORK,
  ...
  it('returns the blockchain error reason: "ERC20: transfer to zero address"', async () => {
    const smartProvider = AetheriumSmartProvider.fromRpcUrl(NETWORK, URL, {
      maxRetries: 1,
    });
    const signer = new Wallet(PK, smartProvider);
