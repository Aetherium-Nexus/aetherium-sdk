...
import { DomainRoutingIsm, TrustedRelayerIsm } from "@aetherium-nexus/core";
import { Address, randomElement, randomInt } from "@aetherium-nexus/utils";
...
import { AetheriumIsmFactory } from "./AetheriumIsmFactory.js";
import {
...
describe("AetheriumIsmFactory", async () => {
  let ismFactoryDeployer: AetheriumProxyFactoryDeployer;
  ...
  let contractsMap: AetheriumContractsMap<ProxyFactoryFactories> = {};
  ...
  before(async () => {
    ...
    ismFactoryDeployer = new AetheriumProxyFactoryDeployer(multiProvider);
    contractsMap = await ismFactoryDeployer.deploy(
      multiProvider.mapKnownChains(() => ({}))
    );
    ismFactory = new AetheriumIsmFactory(contractsMap, multiProvider);
  ...
  for (const type of [IsmType.ROUTING, IsmType.FALLBACK_ROUTING]) {
    ...
    it(`deletes route in an existing ${type} even if not in multiprovider`, async () => {
      ...
      ismFactoryDeployer = new AetheriumProxyFactoryDeployer(multiProvider);
      ismFactory = new AetheriumIsmFactory(
