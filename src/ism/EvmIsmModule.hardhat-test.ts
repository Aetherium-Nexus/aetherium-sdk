...
import { Address, eqAddress } from "@aetherium-nexus/utils";

import { TestChainName, testChains } from "../consts/testChains.js";
import { AetheriumAddresses, AetheriumContracts } from "../contracts/types.js";
...
describe("EvmIsmModule", async () => {
  ...
  let factoryAddresses: AetheriumAddresses<ProxyFactoryFactories>;
  let factoryContracts: AetheriumContracts<ProxyFactoryFactories>;
  ...
  before(async () => {
    ...
    const contractsMap = await new AetheriumProxyFactoryDeployer(
      multiProvider
    ...
    factoryAddresses = Object.keys(factoryContracts).reduce((acc, key) => {
      ...
      return acc;
    }, {} as Record<string, Address>) as AetheriumAddresses<ProxyFactoryFactories>;

    // legacy AetheriumIsmFactory is required to do a core deploy
    const legacyIsmFactory = new AetheriumIsmFactory(
      contractsMap,
      multiProvider
    );
