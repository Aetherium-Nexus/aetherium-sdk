...
import type { AetheriumIsmFactory } from '../ism/AetheriumIsmFactory.js';
...
export abstract class AetheriumDeployer<
  Config,
  Factories extends AetheriumFactories,
> {
  public verificationInputs: ChainMap<ContractVerificationInput[]> = {};
  public cachedAddresses: AetheriumAddressesMap<any> = {};
  public deployedContracts: AetheriumContractsMap<Factories> = {};
  ...
  cacheAddressesMap(addressesMap: AetheriumAddressesMap<any>): void {
  ...
  abstract deployContracts(
    chain: ChainName,
    config: Config,
  ): Promise<AetheriumContracts<Factories>>;

  async deploy(
    configMap: ChainMap<Config>,
  ): Promise<AetheriumContractsMap<Factories>> {
  ...
  protected addDeployedContracts(
    chain: ChainName,
    contracts: AetheriumContracts<any>,
   ...
   * @returns {Promise<AetheriumContracts<Factories>[K]>} A promise that resolves to the deployed contract instance.
   */
  async deployContractWithName<K extends keyof Factories>(
    ...
    >,
    shouldRecover = true,
  ): Promise<AetheriumContracts<Factories>[K]> {
  ...
  async deployProxiedContract<K extends keyof Factories>(
    ...
    proxyAdmin: string,
    constructorArgs: Parameters<Factories[K]['deploy']>,
    initializeArgs?: Parameters<AetheriumContracts<Factories>[K]['initialize']>,
  ): Promise<AetheriumContracts<Factories>[K]> {
