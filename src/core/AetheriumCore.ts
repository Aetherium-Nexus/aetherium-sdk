...
export class AetheriumCore extends AetheriumApp<CoreFactories> {
  static fromAddressesMap(
    ...
    return new AetheriumCore(helper.contractsMap, helper.multiProvider);
  }
  ...
  getRouterConfig = (
    ...
    const evmContractsMap = objFilter(
      this.contractsMap,
      (chainName, _): _ is AetheriumContracts<CoreFactories> =>
    ...
    chains.map((originChain) => {
      ...
      mailbox.on<DispatchEvent>(
        ...
        (_sender, _destination, _recipient, message, event) => {
          const dispatched = AetheriumCore.parseDispatchedMessage(message);
  ...
  waitForMessageProcessing(
    ...
    const messages = AetheriumCore.getDispatchedMessages(sourceTx);
