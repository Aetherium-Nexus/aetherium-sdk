...
import { AetheriumHookDeployer } from "../hook/AetheriumHookDeployer.js";
...
export class AetheriumCoreDeployer extends AetheriumDeployer<
  CoreConfig,
  CoreFactories
> {
  hookDeployer: AetheriumHookDeployer;
  testRecipient: TestRecipientDeployer;

  constructor(
    multiProvider: MultiProvider,
    readonly ismFactory: AetheriumIsmFactory,
    ...
    this.hookDeployer = new AetheriumHookDeployer(
      multiProvider,
      {},
      ismFactory,
      contractVerifier,
  ...
  async deployMailbox(
    ...
    await this.configureIsm(
      ...
      (_mailbox, _module) => _mailbox.populateTransaction.setDefaultIsm(_module)
    );
  ...
  async deployContracts(
    chain: ChainName,
    config: CoreConfig
  ): Promise<AetheriumContracts<CoreFactories>> {
    if (config.remove) {
