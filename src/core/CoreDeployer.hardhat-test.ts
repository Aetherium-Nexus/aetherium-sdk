...
import { Address, objMap, promiseObjAll } from "@aetherium-nexus/utils";
...
import { AetheriumCoreChecker } from "./AetheriumCoreChecker.js";
...
describe("core", async () => {
  ...
  let deployer: AetheriumCoreDeployer;
  let core: AetheriumCore;
  let contracts: AetheriumContractsMap<CoreFactories>;
  ...
  before(async () => {
    ...
    const proxyFactoryDeployer = new AetheriumProxyFactoryDeployer(
      multiProvider
    );
    coreConfig = testCoreConfig(testChains, signer.address);
    const ismFactories = await proxyFactoryDeployer.deploy(coreConfig);
    ismFactory = new AetheriumIsmFactory(ismFactories, multiProvider);
    deployer = new AetheriumCoreDeployer(multiProvider, ismFactory);

    contracts = await deployer.deploy(coreConfig);
    core = new AetheriumCore(contracts, multiProvider);
  });
  ...
  describe("idempotency", () => {
    ...
    it("rotates default and required hooks and recovers artifacts", async () => {
      const getHooks = async (
        contracts: AetheriumContractsMap<CoreFactories>
      ) =>
        promiseObjAll(
  ...
  it("checks", async () => {
    const checker = new AetheriumCoreChecker(
      multiProvider,
      core,
      coreConfig,
      ismFactory,
