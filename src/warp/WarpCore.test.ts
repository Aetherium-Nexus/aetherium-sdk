...
describe('WarpCore', () => {
  ...
  let evmAetNative: Token;
  let evmAetSynthetic: Token;
  let evmAetXERC20: Token;
  let evmAetVSXERC20: Token;
  let evmAetXERC20Lockbox: Token;
  let sealevelAetSynthetic: Token;
  let cwAetCollateral: Token;
  let cw20: Token;
  let cosmosIbc: Token;
  ...
  it('Finds tokens', () => {
    expect(
      warpCore.findToken(test1.name, evmAetNative.addressOrDenom),
    ).to.be.instanceOf(Token);
    expect(
      warpCore.findToken(
        testSealevelChain.name,
        sealevelAetSynthetic.addressOrDenom,
      ),
    ).to.be.instanceOf(Token);
    expect(
      warpCore.findToken(testCosmosChain.name, cw20.addressOrDenom),
    ).to.be.instanceOf(Token);
    expect(warpCore.findToken(test1.name, sealevelAetSynthetic.addressOrDenom))
      .to.be.null;
  });

  it('Gets transfer gas quote', async () => {
    const stubs = warpCore.tokens.map((t) =>
      sinon.stub(t, 'getAetAdapter').returns({
    ...
    await testQuote(evmAetNative, test1.name, TokenStandard.EvmNative);
    await testQuote(
      evmAetNative,
    ...
    await testQuote(evmAetSynthetic, test2.name, TokenStandard.EvmNative);
    await testQuote(
      sealevelAetSynthetic,
      test2.name,
    ...
    await testQuote(cwAetCollateral, test2.name, TokenStandard.CosmosNative, {
      amount: 1n,
      addressOrDenom: 'atom',
    });
  ...
  it('Checks for destination collateral', async () => {
    ...
    await testCollateral(evmAetNative, test2.name, true);
    await testCollateral(evmAetNative, testCosmosChain.name, false);
    await testCollateral(evmAetNative, testSealevelChain.name, true);
    await testCollateral(cwAetCollateral, test1.name, false);
    await testCollateral(evmAetXERC20, testVSXERC20.name, true);
    await testCollateral(evmAetVSXERC20, testXERC20.name, true);
    await testCollateral(evmAetXERC20Lockbox, testXERC20.name, true);
    await testCollateral(evmAetNative, testXERC20Lockbox.name, false);
  ...
  it('Validates transfers', async () => {
    ...
    const validResult = await warpCore.validateTransfer({
      originTokenAmount: evmAetNative.amount(TRANSFER_AMOUNT),
    ...
    const invalidAmount = await warpCore.validateTransfer({
      originTokenAmount: evmAetNative.amount(-10),
      destination: test2.name,
      recipient: MOCK_ADDRESS,
      sender: MOCK_ADDRESS,
    });
    ...
    const insufficientAmount = await warpCore.validateTransfer({
      originTokenAmount: evmAetNative.amount(minimumTransferAmount - 1n),
      destination: test2.name,
      recipient: MOCK_ADDRESS,
      sender: MOCK_ADDRESS,
    });
    ...
    const insufficientBalance = await warpCore.validateTransfer({
      originTokenAmount: evmAetNative.amount(BIG_TRANSFER_AMOUNT),
      destination: test2.name,
      recipient: MOCK_ADDRESS,
      sender: MOCK_ADDRESS,
    });
    ...
    const invalidXERC20LockboxTokenRateLimit = await warpCore.validateTransfer({
      originTokenAmount: evmAetXERC20.amount(BIG_TRANSFER_AMOUNT),
    ...
    const invalidCollateralXERC20LockboxToken = await warpCore.validateTransfer(
      {
        originTokenAmount: evmAetXERC20.amount(MEDIUM_TRANSFER_AMOUNT),
        destination: testXERC20Lockbox.name,
        recipient: MOCK_ADDRESS,
        sender: MOCK_ADDRESS,
      },
  ...
  it('Gets transfer remote txs', async () => {
    ...
    await testGetTxs(evmAetNative, test1.name);
    await testGetTxs(evmAetNative, testCosmosChain.name);
    await testGetTxs(evmAetNative, testSealevelChain.name);
    await testGetTxs(evmAetSynthetic, test2.name);
    await testGetTxs(sealevelAetSynthetic, test2.name, ProviderType.SolanaWeb3);
    await testGetTxs(cwAetCollateral, test1.name, ProviderType.CosmJsWasm);
