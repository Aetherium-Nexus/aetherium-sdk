...
import { IAetXERC20Adapter } from '../token/adapters/ITokenAdapter.js';
...
export class WarpCore {
  ...
  async getInterchainTransferFee({
    ...
    if (defaultQuote) {
      ...
      const aetAdapter = originToken.getAetAdapter(
        this.multiProvider,
        destinationName,
      );
      const destinationDomainId = this.multiProvider.getDomainId(destination);
      const quote = await aetAdapter.quoteTransferRemoteGas(
        destinationDomainId,
        sender,
      );
  ...
  async getTransferRemoteTxs({
    ...
    const aetAdapter = token.getAetAdapter(this.multiProvider, destinationName);
    ...
    if (await this.isApproveRequired({ originTokenAmount, owner: sender })) {
      ...
      const approveTxReq = await aetAdapter.populateApproveTx({
    ...
    const transferTxReq = await aetAdapter.populateTransferRemoteTx({
      weiAmountOrId: amount.toString(),
  ...
  async isDestinationCollateralSufficient({
    ...
    if (
      destinationToken.standard === TokenStandard.EvmAetXERC20Lockbox ||
      destinationToken.standard === TokenStandard.EvmAetVSXERC20Lockbox
    ) {
      const adapter = destinationToken.getAdapter(
        this.multiProvider,
      ) as EvmAetXERC20LockboxAdapter;
  ...
  protected async validateDestinationRateLimit(
    ...
    if (
      destinationToken.standard === TokenStandard.EvmAetVSXERC20 ||
      destinationToken.standard === TokenStandard.EvmAetVSXERC20Lockbox ||
      destinationToken.standard === TokenStandard.EvmAetXERC20 ||
      destinationToken.standard === TokenStandard.EvmAetXERC20Lockbox
    ) {
      const adapter = destinationToken.getAdapter(
        this.multiProvider,
      ) as IAetXERC20Adapter<unknown>;
  ...
  protected async validateOriginCollateral(
    ...
    if (
      originTokenAmount.token.standard === TokenStandard.EvmAetXERC20 ||
      originTokenAmount.token.standard === TokenStandard.EvmAetXERC20Lockbox
    ) {
      const burnLimit = await (
        adapter as IAetXERC20Adapter<unknown>
      ).getBurnLimit();
