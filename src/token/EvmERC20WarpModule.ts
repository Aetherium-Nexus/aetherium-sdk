...
import { buildArtifact as coreBuildArtifact } from "@aetherium-nexus/core/buildArtifact.js";
import {
  Address,
  Domain,
  EvmChainId,
...
import {
  AetheriumModule,
  AetheriumModuleParams,
} from "../core/AbstractAetheriumModule.js";
...
import { AetTokenRouterConfig, AetTokenRouterConfigSchema } from "./types.js";

type WarpRouteAddresses = AetheriumAddresses<ProxyFactoryFactories> & {
  deployedTokenRoute: Address;
};
export class EvmERC20WarpModule extends AetheriumModule<
...
> {
  ...
  constructor(
    ...
    args: AetheriumModuleParams<AetTokenRouterConfig, WarpRouteAddresses>,
   ...
   */
  async read(): Promise<AetTokenRouterConfig> {
   ...
   */
  async update(
    expectedConfig: AetTokenRouterConfig
  ): Promise<AnnotatedEV5Transaction[]> {
    AetTokenRouterConfigSchema.parse(expectedConfig);
  ...
  createEnrollRemoteRoutersUpdateTxs(
    actualConfig: AetTokenRouterConfig,
    expectedConfig: AetTokenRouterConfig
  ...
  async deployNewHook(expectedConfig: AetTokenRouterConfig): Promise<{
    deployedHook: Address;
    updateTransactions: AnnotatedEV5Transaction[];
  }> {
    this.logger.info(
   ...
   * @returns A new instance of the EvmERC20WarpAetheriumModule.
   */
  static async create(params: {
    ...
    proxyFactoryFactories: AetheriumAddresses<ProxyFactoryFactories>;
    ...
    const deployer = new AetERC20Deployer(multiProvider);
