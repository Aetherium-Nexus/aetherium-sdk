...
export async function expandWarpDeployConfig(
  ...
  const derivedTokenMetadata = await AetERC20Deployer.deriveTokenMetadata(
    multiProvider,
    warpDeployConfig
  );
...
/**
 * transforms the provided {@link AetTokenRouterConfig}, removing the address, totalSupply and ownerOverrides
 * field where they are not required for the config comparison
 */
export function transformConfigToCheck(
  obj: AetTokenRouterConfig
): AetTokenRouterConfig {
