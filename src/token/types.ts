 ...
 */
export const AetTokenConfigSchema = z.discriminatedUnion("type", [
...
export type AetTokenConfig = z.infer<typeof AetTokenConfigSchema>;

export const AetTokenRouterConfigSchema = AetTokenConfigSchema.and(
  GasRouterConfigSchema
);
export type AetTokenRouterConfig = z.infer<typeof AetTokenRouterConfigSchema>;

const AetTokenRouterConfigMailboxOptionalSchema = AetTokenConfigSchema.and(
  GasRouterConfigSchema.extend({
    mailbox: z.string().optional(),
  })
);

export type AetTokenRouterConfigMailboxOptional = z.infer<
  typeof AetTokenRouterConfigMailboxOptionalSchema
>;

export const WarpRouteDeployConfigSchema = z
  .record(AetTokenRouterConfigMailboxOptionalSchema)
  .refine((configMap) => {
  ...
  .transform((warpRouteDeployConfig, ctx) => {
    ...
    if (
      ...
      return objMap(warpRouteDeployConfig, (_, config) => {
        ...
        return config;
      }) as Record<string, AetTokenRouterConfig>;
    }
...
function getCCIPConfigMaps(
  warpRouteDeployConfig: Record<string, AetTokenRouterConfigMailboxOptional>
): {
