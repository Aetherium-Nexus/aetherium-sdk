...
interface AetTokenAddresses {
  token: Address;
  warpRouter: Address;
  mailbox: Address;
}

export abstract class SealevelAetTokenAdapter
  extends SealevelTokenAdapter
  implements IAetTokenAdapter<Transaction>
{
  public readonly warpProgramPubKey: PublicKey;
  public readonly addresses: AetTokenAddresses;
  protected cachedTokenAccountData: SealevelAetheriumTokenData | undefined;

  constructor(
    public readonly chainName: ChainName,
    public readonly multiProvider: MultiProtocolProvider,
    addresses: AetTokenAddresses,
    public readonly isSpl2022: boolean = false
  ) {
  ...
  async getTokenAccountData(): Promise<SealevelAetheriumTokenData> {
    if (!this.cachedTokenAccountData) {
      const tokenPda = this.deriveAetTokenAccount();
      ...
      this.cachedTokenAccountData =
        wrappedData.data as SealevelAetheriumTokenData;
    }
  ...
  async populateTransferRemoteTx({
    ...
    const value = new SealevelInstructionWrapper({
      instruction: SealevelAetTokenInstruction.TransferRemote,
  ...
  getTransferInstructionKeyList({
    ...
    let keys = [
      ...
      {
        pubkey: this.deriveAetTokenAccount(),
        isSigner: false,
        isWritable: false,
      },
  ...
  deriveMailboxOutboxAccount(mailbox: PublicKey): PublicKey {
    return super.derivePda(["aetherium", "-", "outbox"], mailbox);
  }

  deriveMessageDispatchAuthorityAccount(): PublicKey {
    return super.derivePda(
      ["aetherium_dispatcher", "-", "dispatch_authority"],
  ...
  deriveAetTokenAccount(): PublicKey {
    return super.derivePda(
      ["aetherium_message_recipient", "-", "handle", "-", "account_metas"],
      this.warpProgramPubKey
    );
  }
  ...
  protected getIgpAdapter(
    tokenData: SealevelAetheriumTokenData
...
// Interacts with Aet Native token programs
export class SealevelAetNativeAdapter extends SealevelAetTokenAdapter {
  ...
  constructor(
    ...
    public readonly multiProvider: MultiProtocolProvider,
    addresses: {
      // A 'token' address is not required for aet native tokens (e.g. aetSOL)
      token?: Address;
      warpRouter: Address;
      mailbox: Address;
    },
  ...
  deriveNativeTokenCollateralAccount(): PublicKey {
    return super.derivePda(
      ["aetherium_token", "-", "native_collateral"],
      this.warpProgramPubKey
    );
  }
...
// Interacts with Aet Collateral token programs
export class SealevelAetCollateralAdapter extends SealevelAetTokenAdapter {
  async getBalance(owner: Address): Promise<bigint> {
  ...
  deriveEscrowAccount(): PublicKey {
    return super.derivePda(
      ["aetherium_token", "-", "escrow"],
      this.warpProgramPubKey
    );
  }
}

// Interacts with Aet Synthetic token programs (aka 'AetTokens')
export class SealevelAetSyntheticAdapter extends SealevelAetTokenAdapter {
  override getTransferInstructionKeyList(
    params: KeyListParams
  ): Array<AccountMeta> {
    return [
