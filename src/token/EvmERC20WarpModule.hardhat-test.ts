...
import { randomInt } from "@aetherium-nexus/utils";
...
import { AetTokenRouterConfig } from "./types.js";
...
describe("EvmERC20WarpAetheriumModule", async () => {
  ...
  let ismFactory: AetheriumIsmFactory;
  let factories: AetheriumContractsMap<ProxyFactoryFactories>;
  let ismFactoryAddresses: AetheriumAddresses<ProxyFactoryFactories>;
  ...
  before(async () => {
    ...
    ismFactory = new AetheriumIsmFactory(factories, multiProvider);
  ...
  it("should create with a collateral config", async () => {
    const config: AetTokenRouterConfig = {
      ...baseConfig,
  ...
  it("should create with a collateral vault config", async () => {
    ...
    const collateralVaultContract = AetERC4626Collateral__factory.connect(
      deployedTokenRoute,
      signer
    );
  ...
  it("should create with a synthetic config", async () => {
    ...
    // Validate onchain token values
    const syntheticContract = AetERC20__factory.connect(
      deployedTokenRoute,
      signer
    );
  ...
  it("should create with a native config", async () => {
    ...
    // Validate onchain token values
    const nativeContract = AetNative__factory.connect(
      deployedTokenRoute,
      signer
    );
  ...
  describe("Update", async () => {
    ...
    it("should deploy and set a new Ism", async () => {
      ...
      for (const interchainSecurityModule of ismConfigToUpdate) {
        const expectedConfig: AetTokenRouterConfig = {
          ...actualConfig,
    ...
    it("should update the ProxyAdmin owner only if they are different", async () => {
      ...
      const newOwner = randomAddress();
      const updatedWarpCoreConfig: AetTokenRouterConfig = {
        ...config,
        proxyAdmin: {
      ...
      const latestConfig: AetTokenRouterConfig = normalizeConfig(
