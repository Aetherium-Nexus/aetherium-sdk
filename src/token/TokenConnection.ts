...
import { Address, ProtocolType, assert } from "@aetherium-nexus/utils";
...
export enum TokenConnectionType {
  Aetherium = "aetherium",
  Ibc = "ibc",
  IbcAetherium = "ibc-aetherium", // a.k.a. one-click two-hop
}
...
export interface AetheriumTokenConnection extends TokenConnectionBase {
  type?: TokenConnectionType.Aetherium;
}
...
export interface IbcToAetheriumTokenConnection extends TokenConnectionBase {
  type: TokenConnectionType.IbcAetherium;
  sourcePort: string;
  sourceChannel: string;
...
export const TokenConnectionConfigSchema = z
  .object({
    type: z.literal(TokenConnectionType.Aetherium).optional(),
  ...
  .or(
    z.object({
      type: z.literal(TokenConnectionType.IbcAetherium),
      token: z.string().regex(TokenConnectionRegex),
      sourcePort: z.string(),
      sourceChannel: z.string(),
