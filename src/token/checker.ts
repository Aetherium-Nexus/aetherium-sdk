...
import { eqAddress, objMap } from "@aetherium-nexus/utils";
...
import { AetERC20Factories } from "./contracts.js";
import {
...
export class AetERC20Checker extends ProxiedRouterChecker<
...
> {
  ...
  async checkToken(chain: ChainName): Promise<void> {
    ...
    const expectedConfig = this.configMap[chain];
    const aetToken = this.app.router(this.app.getContracts(chain));
    ...
    if (isNativeTokenConfig(expectedConfig)) {
      try {
        ...
        const violation: TokenMismatchViolation = {
          ...
          expected: "true",
          actual: "false",
          tokenAddress: aetToken.address,
        };
        this.addViolation(violation);
      }
    } else if (isSyntheticTokenConfig(expectedConfig)) {
      await checkERC20(aetToken as unknown as ERC20, expectedConfig);
    } else if (
      ...
      const actualToken = await (
        aetToken as unknown as AetERC20Collateral
      ).wrappedToken();
  ...
  async getActualDecimals(
    ...
    if (isNativeTokenConfig(expectedConfig)) {
      ...
      decimals = await (aetToken as unknown as ERC20).decimals();
    } else if (
      isCollateralTokenConfig(expectedConfig) ||
      isXERC20TokenConfig(expectedConfig)
    ) {
