...
import { AetheriumReader } from "../utils/AetheriumReader.js";
...
export class EvmERC20WarpRouteReader extends AetheriumReader {
  ...
  /**
   * Derives the configuration for a Aetherium ERC20 router contract at the given address.
   *
   * @param warpRouteAddress - The address of the Aetherium ERC20 router contract.
   * @returns The configuration for the Aetherium ERC20 router.
   *
   */
  async deriveWarpRouteConfig(
    warpRouteAddress: Address
  ): Promise<AetTokenRouterConfig> {
  ...
  async deriveTokenType(warpRouteAddress: Address): Promise<TokenType> {
    ...
    > = {
      [TokenType.collateralVaultRebase]: {
        factory: AetERC4626Collateral__factory,
        method: "NULL_RECIPIENT",
      },
      [TokenType.collateralVault]: {
        factory: AetERC4626OwnerCollateral__factory,
        method: "vault",
      },
      [TokenType.XERC20Lockbox]: {
        factory: AetXERC20Lockbox__factory,
        method: "lockbox",
      },
      [TokenType.collateral]: {
        factory: AetERC20Collateral__factory,
        method: "wrappedToken",
      },
      [TokenType.syntheticRebase]: {
        factory: AetERC4626__factory,
        method: "collateralDomain",
      },
      [TokenType.synthetic]: {
        factory: AetERC20__factory,
        method: "decimals",
      },
    };
  ...
  async fetchMailboxClientConfig(
    ...
    const warpRoute = AetERC20Collateral__factory.connect(
      routerAddress,
      this.provider
    );
  ...
  async fetchTokenConfig(
    ...
    if (
      ...
      if (type === TokenType.XERC20Lockbox) {
        ...
        const aetXERC20Lockbox = AetXERC20Lockbox__factory.connect(
          warpRouteAddress,
          this.provider
        );
        xerc20Token = await aetXERC20Lockbox.xERC20();
        token = xerc20Token;
        lockbox = await aetXERC20Lockbox.lockbox();
      } else {
        const erc20 = AetERC20Collateral__factory.connect(
          warpRouteAddress,
          this.provider
        );
      ...
      if (type === TokenType.syntheticRebase) {
        const aetERC4626 = AetERC4626__factory.connect(
          warpRouteAddress,
          this.provider
        );
        const collateralChainName = this.multiProvider.getChainName(
          await aetERC4626.collateralDomain()
        );
  ...
  async fetchERC20Metadata(tokenAddress: Address): Promise<TokenMetadata> {
    const erc20 = AetERC20__factory.connect(tokenAddress, this.provider);
